...
class Geometry(Tidy3dBaseModel, ABC):
    ...
    def _inds_inside_bounds(
        ...
        bounds = self.bounds
        inds_in = []
        for dim, coords in enumerate([x_1d, y_1d, z_1d]):
    ...
    def inside_meshgrid(
        ...
        -------
        np.ndarray[bool]
            Array with shape ``(x_1d.size, y_1d.size, z_1d.size)``, which is ``True`` for every
    ...
    def intersects_axis_position(self, axis: int, position: float) -> bool:
        ...
        bool
            Whether this geometry intersects the plane.
        """
        if self.bounds[0][axis] <= position <= self.bounds[1][axis]:
            return True
        return False
...
class GeometryGroup(Geometry):
    ...
    def inside_meshgrid(
        ...
        individual_insides = (geom.inside_meshgrid(x_1d, y_1d, z_1d) for geom in self.geometries)
