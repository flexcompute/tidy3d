...
from ..simulation import Simulation, AbstractYeeGridSimulation
...
from .monitor import EMEMonitor, EMEMonitorType, EMEFieldMonitor
...
class EMESimulation(AbstractYeeGridSimulation):
    ...
    @cached_property
    def _grid_spec_with_wavelength(self) -> GridSpec:
        """Update :class:`.AutoGrid` to have wavelength based on simulation ``freqs``."""
        freqs = self.freqs
        ...
        if grid_spec.auto_grid_used and grid_spec.wavelength is None:
            ...
            return grid_spec.updated_copy(wavelength=wavelength)
        return grid_spec
    ...
    def grid(self) -> Grid:
        ...
        grid = self._grid_spec_with_wavelength.make_grid(
