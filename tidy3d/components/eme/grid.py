...
from ..types import Axis, Coordinate, Size, annotate_type
...
class EMEUniformGrid(EMEGridSpec):
    ...
    >>> mode_spec = ModeSpec(num_modes=10)
...
class EMEExplicitGrid(EMEGridSpec):
    ...
    Example
    -------
    >>> from tidy3d import EMEExplicitGrid, ModeSpec
    >>> mode_spec1 = ModeSpec(num_modes=10)
    >>> mode_spec2 = ModeSpec(num_modes=20)
    ...
    mode_specs: List[ModeSpec] = pd.Field(
        ...,
...
class EMECompositeGrid(EMEGridSpec):
    ...
    Example
    -------
    >>> from tidy3d import EMEUniformGrid, ModeSpec
    >>> mode_spec1 = ModeSpec(num_modes=10)
    >>> mode_spec2 = ModeSpec(num_modes=20)
    ...
    def subgrid_bounds(
        ...
        sim_rmin = center[axis] - size[axis] / 2
        sim_rmax = center[axis] + size[axis] / 2
        if self.subgrid_boundaries[0] < sim_rmin:
            raise ValidationError(
                "The first item in 'subgrid_boundaries' is outside the simulation domain."
            )
        if self.subgrid_boundaries[-1] > sim_rmax:
            raise ValidationError(
...
class EMEGrid(Box):
    ...
    @pd.validator("mode_specs", always=True)
    def _validate_mode_specs(cls, val):
        """Check consistency of mode_specs."""
        for ind, mode_spec in enumerate(val):
            if mode_spec.angle_theta != 0 or mode_spec.angle_phi != 0:
                raise ValidationError(
                    f"Mode spec at 'mode_specs[{ind}]' "
                    "has 'angle_theta != 0' or 'angle_phi != 0'. "
                    "This is not supported in the EME grid. "
                    "To achieve off-normal injection, add an additional "
                    "'ModeSolverMonitor' to the EME simulation, and then use "
                    "'EMESimulationData.smatrix_in_basis' to inject these modes "
                    "into the EME simulation."
                )
            if mode_spec.precision == "double":
                raise ValidationError(
                    "Double precision mode solving is not " "currently supported in EME."
                )
        return val
