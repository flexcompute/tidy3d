...
class EMESimulationData(AbstractYeeGridSimulationData):
    ...
    def smatrix_in_basis(
        ...
        for freq_ind, freq in enumerate(f):
            for sweep_index_curr in sweep_index:
                S11 = self.smatrix.S11.sel(f=freq, sweep_index=sweep_index_curr).to_numpy()
                S12 = self.smatrix.S12.sel(f=freq, sweep_index=sweep_index_curr).to_numpy()
                S21 = self.smatrix.S21.sel(f=freq, sweep_index=sweep_index_curr).to_numpy()
                S22 = self.smatrix.S22.sel(f=freq, sweep_index=sweep_index_curr).to_numpy()

                O1 = (
                    overlaps1.sel(f=freq)
                    # .sel(mode_index_1=self.smatrix.S11.mode_index_in)
                    .to_numpy()
                )
                O2 = (
                    overlaps2.sel(f=freq)
                    # .sel(mode_index_1=self.smatrix.S22.mode_index_in)
                    .to_numpy()
                )
    ...
    def field_in_basis(
        ...
        overlaps = modes.outer_dot(port_modes)
        ...
        for field_key, field_comp in field.field_components.items():
            ...
            data[:] = np.nan
            field_comp_data = field_comp.to_numpy()
            for i in range(len(mode_index)):
                data[:, :, :, :, i, 0] = 0
                for j in range(len(overlaps.mode_index_1)):
                    data[:, :, :, :, i, 0] += (
                        overlap[None, None, None, :, i, j] * field_comp_data[:, :, :, :, j, 0]
                    )
            coords = dict(
