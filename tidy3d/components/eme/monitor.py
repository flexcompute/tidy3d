...
class EMEMonitor(AbstractMonitor, ABC):
    ...
    mode_indices: Optional[List[pd.NonNegativeInt]] = pd.Field(
        None,
        title="Mode Indices",
        description="Mode indices at which the monitor will record.",
    )
...
class EMEModeSolverMonitor(EMEMonitor):
    ...
    interval_space: Tuple[pd.PositiveInt, pd.PositiveInt, pd.PositiveInt] = pd.Field(
        ...
        "in the propagation direction is not used.",
    )
