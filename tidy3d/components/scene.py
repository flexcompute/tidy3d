...
class Scene(Box):  # pylint:disable=too-many-public-methods
    """Contains geometric information to use in simulations.
    ...
    def _set_plot_bounds(self, ax: Ax, x: float = None, y: float = None, z: float = None) -> Ax:
        ...
        _, (xmin, ymin) = self.pop_axis(self.bounds[0], axis=axis)
        _, (xmax, ymax) = self.pop_axis(self.bounds[1], axis=axis)
    ...
    @equal_aspect
    @add_ax_if_none
    def plot_heat_conductivity(  # pylint:disable=too-many-arguments
        self,
        x: float = None,
        y: float = None,
        z: float = None,
        ...
        ax = self.plot_structures_heat_conductivity(cbar=cbar, alpha=alpha, ax=ax, x=x, y=y, z=z)
        ax = self._set_plot_bounds(ax=ax, x=x, y=y, z=z)
        return ax

    @equal_aspect
    @add_ax_if_none
    def plot_structures_heat_conductivity(  # pylint: disable=too-many-arguments,too-many-locals
        self,
        x: float = None,
        y: float = None,
        z: float = None,
