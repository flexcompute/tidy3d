...
class AbstractModeMonitor(PlanarMonitor, FreqMonitor):
    ...
    store_fields: bool = pydantic.Field(
        True,
        title="Store Fields",
        description="Flag indicating whether mode field profiles should be stored with the "
        "corresponding data.",
    )
...
class ModeSolverMonitor(AbstractModeMonitor):
    ...
    def deprecation_warning(cls, values):
        ...
        log.warning(
            "ModeSolverMonitor will be deprecated in the future and can now be replaced by a "
            "ModeMonitor with the attribute 'store_fields' set to True.",
            log_once=True,
        )
