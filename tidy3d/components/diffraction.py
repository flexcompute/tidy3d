...
from .data.monitor_data import DiffractionData
from .monitor import BYTES_COMPLEX
...
def sim_bloch_vecs(
    ...
    if len(fixed_angle_sources) > 0:
        ...
        medium_set = Scene.intersecting_media(src, total_structures)
...
def _generate_orders(
    ...
    while next_order:
        ...
        order_pos = DiffractionData.shifted_orders((order,), bloch)
        pos_vec = handle_zero_division(order_pos * 2.0 * np.pi / wave_numbers, domain_size)

        order_neg = DiffractionData.shifted_orders((-order,), bloch)
...
def diffraction_order_grid_axes(
    ...
    bloch_x = DiffractionData.shifted_orders(tuple(int(order) for order in orders_x), bloch_x_0)
    bloch_y = DiffractionData.shifted_orders(tuple(int(order) for order in orders_y), bloch_y_0)
