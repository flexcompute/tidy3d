...
class Simulation(AbstractYeeGridSimulation):
    ...
    *   `High-Q silicon resonator <../../notebooks/HighQSi.html>`_
    ...
    def _warn_monitor_simulation_frequency_range(cls, val, values):
        ...
        freq_range_min = min((freq_range[0] for freq_range in source_ranges), default=0.0)
        freq_range_max = max((freq_range[1] for freq_range in source_ranges), default=0.0)
    ...
    def _validate_monitor_simulation_frequency_range(self) -> None:
        ...
        for monitor in self.monitors:
            ...
            if freqs.min() < freq_range_min or freqs.max() > freq_range_max:
                raise SetupError(
                    f"Frequency-domain monitor '{monitor.name}' contains frequencies "
                    f"outside of the simulation frequency range ({freq_range_min:2e}, {freq_range_max:2e})"
                    "(Hz) as defined by the sources."
                )
