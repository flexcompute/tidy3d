...
def _get_url(
    ...
    gui_type: Literal["default", "rf"] = "default",
) -> str:
    ...
    if task_id:
        ...
        raise WebError("Required URL resource ID.")
    if gui_type == "rf":
...
def upload(
    ...
    if (task_type == "COMPONENT_MODELER") or (
        task_type == "TERMINAL_COMPONENT_MODELER"
    ):  # TODO REMOVE
        task_type = "RF"
...
def download(
    ...
    task = SimulationTask(taskId=task_id, batch_id=batch_id)
...
def load(
    ...
    if not os.path.exists(path) or replace_existing:
        download(task_id=task_id, batch_id=batch_id, path=path, verbose=verbose, progress_callback=progress_callback)
