...
# PEC permittivity cut-off value. Let it be as large as possible without causing overflow in
# double precision
PEC_CUT_OFF = 1e70
...
class EigSolver(Tidy3dBaseModel):
    ...
    def solver_em(
        ...
        # PEC should be visible in low-frequency/high resolution, so it shall
        # be scaled as max(1/k0 dl) **2
        pec_scaling = max([np.max(abs(f)) for f in der_mats]) ** 2
        ...
        def conductivity_model_for_pec(eps, threshold=0.9 * pec_val):
            ...
            eps[eps <= threshold] = 1 + 1j * pec_scaled_val
            return eps
    ...
    def solver_diagonal(
        ...
        # P = p_mu + p_diff
        # Q = q_ep + q_diff
        # Note that p_diff @ q_diff = 0, so that PQ = p_mu @ Q + p_diff @ q_ep
        p_mu = sp.bmat(
