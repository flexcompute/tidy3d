...
class WavePort(AbstractTerminalPort, Box):
    ...
    mode_spec: ModeSpec = pd.Field(ModeSpec())
    mode_index: pd.NonNegativeInt = pd.Field(0)
    voltage_integral: Optional[VoltageIntegralTypes] = pd.Field(None)
    current_integral: Optional[CurrentIntegralTypes] = pd.Field(None)
    num_grid_cells: Optional[int] = pd.Field(
        DEFAULT_WAVE_PORT_NUM_CELLS, ge=MIN_WAVE_PORT_NUM_CELLS
    )
