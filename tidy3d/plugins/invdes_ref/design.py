...
class AbstractInverseDesign(InvdesBaseModel, abc.ABC):
    ...
    def make_objective_fn(
        ...
        def objective_fn(params: anp.ndarray, aux_data: dict = None) -> float:
            ...
            penalty_value = np.sum(
                self.design_regions[idx].penalty_value(params[idx])
                for idx in range(0, len(self.design_regions))
            )  # self.design_region.penalty_value(params)
...
class InverseDesign(AbstractInverseDesign):
    ...
    def to_simulation(self, params: anp.ndarray) -> td.Simulation:
        ...
        design_region_structures = [
            self.design_regions[idx].to_structure(params[idx])
            for idx in range(0, len(self.design_regions))
        ]
        ...
        for region in self.design_regions:
            mesh_override_structure = region.mesh_override_structure
        ...
        return self.simulation.updated_copy(
            structures=list(self.simulation.structures) + design_region_structures,
            grid_spec=grid_spec,
        )
