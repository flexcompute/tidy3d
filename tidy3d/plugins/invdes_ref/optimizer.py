...
class Optimizer(InvdesBaseModel):
    ...
    def initialize_result(
        ...
        if params0 is not None:
            ...
            params0 = anp.concatenate(
                (
                    anp.expand_dims(region.initial_parameters, axis=0)
                    for region in self.design.design_regions
                ),
                axis=0,
            )
        state = [
            self.design.design_regions[idx].optimization_spec.initial_state(params0[idx])
            for idx in range(0, len(self.design.design_regions))
        ]  # self.initial_state(params0)

        # print(tuple(params0))
        tuple_params = tuple(map(tuple, params0))
        # print(tuple_params)
        # initialize empty result
        return InverseDesignResult(design=self.design, opt_state=[state], params=(tuple_params,))
    ...
    def continue_run(
        ...
        opt_state = list(result.get_last("opt_state"))
        params = list(result.get_last("params"))
        ...
        for step_index in range(done_steps, done_steps + num_steps):
            ...
            for idx in range(0, len(self.design.design_regions)):
                params[idx][:], opt_state[idx] = self.design.design_regions[
                    idx
                ].optimization_spec.update(
                    parameters=params[idx], state=opt_state[idx], gradient=-grad[idx]
                )
            ...
            # grad_tuple = tuple(map(tuple, grad))

            tuple_params = tuple(map(tuple, params))
            tuple_grad = tuple(map(tuple, grad))

            for key, value in zip(
                ("params", "opt_state", "grad"), (tuple_params, opt_state, tuple_grad)
            ):
