...
class MethodSample(Method, ABC):
    ...
    def _run(
        self, parameters: tuple[ParameterType, ...], run_fn: Callable, console
    ) -> tuple[list[dict[str, Any]], list[float], list[Any] | None, Any | None]:
        """Defines the search algorithm."""
...
class MethodGradientBased(MethodOptimize, ABC):
    """A standard method for performing gradient-based optimization, built around the `scipy.optimize` package.
    The fitness function is maximising by default.

    Example
    -------
    >>> import tidy3d.plugins.design as tdd
    >>> method = tdd.MethodGradientBased(max_iter=100, tolerance=1e-6)
    """

    max_iter: pd.PositiveInt = pd.Field(
        ...,
        title="Maximum Iterations",
        description="The maximum number of iterations to run the gradient-based optimization.",
    )

    tolerance: pd.PositiveFloat = pd.Field(
        default=1e-6,
        title="Tolerance",
        description="The tolerance for convergence in the gradient-based optimization.",
    )

    method: Literal["adam"] = pd.Field(
        default="adam",
        title="Optimization Method",
        description="The gradient-based optimization method to use. Currently supports 'adam'.",
    )

    step_size: pd.PositiveFloat = pd.Field(
        default=0.01,
        title="Learning Rate",
        description="The learning rate (step size) for the Adam optimizer.",
    )

    beta1: pd.confloat(ge=0, le=1) = pd.Field(
        default=0.9,
        title="Beta1",
        description="Exponential decay rate for the first moment estimates.",
    )

    beta2: pd.confloat(ge=0, le=1) = pd.Field(
        default=0.999,
        title="Beta2",
        description="Exponential decay rate for the second moment estimates.",
    )

    epsilon: pd.PositiveFloat = pd.Field(
        default=1e-8,
        title="Epsilon",
    ...
    def _get_run_count(self, parameters: Optional[list] = None) -> int:
        """Return the maximum number of runs for the method based on current method arguments."""
        # Estimate based on max iterations and parameters (each iteration may evaluate multiple points)
        if parameters is None:
            return self.max_iter * 2  # Conservative estimate
        return self.max_iter * (
            len(parameters) + 1
        )  # +1 for function evaluation, *len(params) for gradients

    def _run(
        self, parameters: tuple[ParameterType, ...], run_fn: Callable, console
    ) -> tuple[list[dict[str, Any]], list[float], list[Any] | None, Any | None]:
        """Defines the gradient-based optimization algorithm for the method."""
        ...
        # Filter to only use ParameterFloat parameters for gradient-based optimization
        float_parameters = [param for param in parameters if isinstance(param, ParameterFloat)]
        other_parameters = [param for param in parameters if not isinstance(param, ParameterFloat)]

        if not float_parameters:
            raise ValueError(
                "No ParameterFloat parameters found. Gradient-based optimization requires at least one continuous parameter."
            )

        if other_parameters and console is not None:
            console.log(
                f"Warning: Ignoring non-float parameters for gradient-based optimization: {[p.name for p in other_parameters]}"
            )

        # Build parameter bounds and initial values for float parameters only
        bounds = []
        x0 = []

        for param in float_parameters:
            bounds.append(param.span)
            x0.append((param.span[0] + param.span[1]) / 2)  # Midpoint as starting point

        # Store all evaluated points and results
        fn_args = []
        results = []
        aux_values = []

        # Create a wrapper that converts numpy array to parameter dict and calls run_fn
        def objective_wrapper(x):
            """Convert numpy array to parameter dict and call run_fn."""
            # Convert numpy array x to parameter dictionary (only for float parameters)
            param_dict = {}
            for i, param in enumerate(float_parameters):
                param_dict[param.name] = float(x[i])

            # Add default values for non-float parameters
            for param in other_parameters:
                if isinstance(param, ParameterInt):
                    param_dict[param.name] = param.span[0]  # Use minimum value
                elif isinstance(param, ParameterAny):
                    param_dict[param.name] = param.allowed_values[0]  # Use first allowed value

            # Call run_fn with the parameter dict
            result, aux_out = self._extract_output(run_fn([param_dict]))
            fitness = result[0]

            # Store results
            fn_args.append(param_dict)
            results.append(fitness)
            self._flatten_and_append(aux_out, aux_values)

            return -fitness  # Negative for maximization

        # Create gradient function using autograd
        value_and_grad_func = value_and_grad(objective_wrapper)

        # Initialize
        x = np.array(x0, dtype=float)

        if console is not None:
            console.log(f"Starting gradient-based optimization with {self.max_iter} iterations")
            console.log(
                f"Optimizing {len(float_parameters)} float parameters: {[p.name for p in float_parameters]}"
            )

        # Main optimization loop
        for _ in range(self.max_iter):
            # Evaluate function and gradient using autograd
            current_value, grad = value_and_grad_func(x)
            ...
            for i, (min_val, max_val) in enumerate(bounds):
                x_new[i] = np.clip(x_new[i], min_val, max_val)

            x = x_new

        return fn_args, results, aux_values, None
