...
from tidy3d._common.components.autograd import TracedArrayFloat2D, get_static
from tidy3d._common.components.autograd.types import TracedFloat
from tidy3d._common.components.autograd.utils import hasbox
from tidy3d._common.components.base import cached_property
from tidy3d._common.components.geometry import base, triangulation
from tidy3d._common.components.transformation import ReflectionFromPlane, RotationAroundAxis
from tidy3d._common.config import config
from tidy3d._common.constants import LARGE_NUMBER, MICROMETER, fp_eps
from tidy3d._common.exceptions import SetupError, Tidy3dImportError, ValidationError
...
if TYPE_CHECKING:
    ...
    from tidy3d._common.components.autograd import AutogradFieldMap
    from tidy3d._common.components.autograd.derivative_utils import DerivativeInfo
...
class PolySlab(base.Planar):
    ...
    def no_self_intersecting_polygon_during_extrusion(self: Self) -> Self:
        ...
        if math.isclose(getval(self.sidewall_angle), 0):
            return self
    ...
    def _edge_events_detection(
        ...
        for dist in dist_list:
            ...
            offset_vertices = PolySlab._proper_vertices(poly_offset.exterior.coords)
            ...
            if poly_offset_back.geom_type == "MultiPolygon" or len(poly_offset_back.interiors) > 0:
                return True
            offset_back_vertices = poly_offset_back.exterior.coords
    ...
    def sub_polyslabs(self) -> list[PolySlab]:
        ...
        for dist_val in self._dilation_length:
            ...
            while not math.isclose(dist_now, dist_val):
                ...
                sub_polyslab_list.append(PolySlab.model_validate(sub_polyslab_dict))
