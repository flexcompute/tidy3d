...
from tidy3d._common.web.core import http_util
from tidy3d._common.web.core.cache import FOLDER_CACHE
...
from tidy3d._common.web.core.core_config import get_logger_console
from tidy3d._common.web.core.exceptions import WebError, WebNotFoundError
from tidy3d._common.web.core.file_util import read_simulation_from_hdf5
from tidy3d._common.web.core.http_util import get_version as _get_protocol_version
from tidy3d._common.web.core.http_util import http
from tidy3d._common.web.core.s3utils import download_file, download_gz_file, upload_file
from tidy3d._common.web.core.task_info import BatchDetail, TaskInfo
from tidy3d._common.web.core.types import (
    PayType,
    Queryable,
    ResourceLifecycle,
    Submittable,
...
if TYPE_CHECKING:
    ...
    from tidy3d._common.web.core.stub import TaskStub
...
class SimulationTask(WebTask):
    ...
    def get(cls, task_id: str, verbose: bool = True) -> SimulationTask:
        ...
        try:
            ...
            log.error(f"The requested task ID '{task_id}' does not exist.")
            raise e
...
class BatchTask(WebTask):
    ...
    def get(cls, task_id: str, verbose: bool = True) -> BatchTask:
        ...
        try:
            ...
            log.error(f"The requested batch ID '{task_id}' does not exist.")
            raise e
