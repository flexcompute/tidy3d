...
def _check_simulation_types(simulations: list[WorkflowType]) -> None:
    """Ensure simulation types are consistent for autograd and modeler compatibility.

    Rules
    -----
    - If at least one simulation is of ComponentModelerType (and not valid for autograd), all submitted simulations have to be like that).
    """
    has_modeler = any(isinstance(sim, ComponentModelerType) for sim in simulations)

    if has_modeler and len(simulations) > 1:
        raise ValueError(
            "web.run() does not support ComponentModelerTypes when submitting multiple simulations."
        )
...
def run(
    ...
    **Lazy loading**
    ...
    ------
    ValueError
        If no simulations are found in ``simulation`` or component modeler simulation is provided along other simulations.
    TypeError
    ...
    _check_simulation_types(list(h2sim.values()))
