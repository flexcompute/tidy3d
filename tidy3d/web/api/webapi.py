...
def start(
    ...
    if _is_modeler_batch(task_id):
        ...
        if status_str in ("validate_success", "validate_warn"):
            if verbose:
                console.log(f"Component Modeler '{task_id}' validation succeeded.")
                ...
                for name, t in name_to_task.items():
                    if name not in task_bars:
                        ...
                        pbar = progress.add_task(
                            f"  {name}",
                            total=len(VALID_PROGRESS_STATES) - 1,
                            completed=min(idx, len(VALID_PROGRESS_STATES) - 1),
                        )
                        task_bars[name] = pbar
...
def estimate_cost(
    ...
    if _is_modeler_batch(task_id):
        ...
        while status not in ALL_STATES:
            time.sleep(REFRESH_TIME)
            d = _batch_detail(task_id)
            status = d.totalStatus.value

        if status in VALID_PROGRESS_STATES:
...
def postprocess_start(
    ...
    # If the run is complete, trigger the postprocess phase and return
    if console:
        console.log("Run phase complete. Triggering postprocessing... ✅")
