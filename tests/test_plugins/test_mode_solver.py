...
def test_mode_solver_simple(mock_remote_api, local):
    ...
    if local:
        freqs = [td.frequency(0.9), td.frequency(1.0), td.frequency(1.1)]
    else:
        freqs = [td.frequency(1.0)]
    ms = ModeSolver(
        simulation=simulation,
...
def test_mode_solver_2D():
    ...
    ms = ModeSolver(
        simulation=simulation, plane=PLANE, mode_spec=mode_spec, freqs=[td.frequency(1.0)]
    )
    compare_colocation(ms)
    verify_pol_fraction(ms)
    check_ms_reduction(ms)
...
def test_group_index(mock_remote_api, log_capture, local):
    ...
    if local:
        freqs = [td.frequency(1.54), td.frequency(1.55), td.frequency(1.56)]
    else:
        freqs = [td.frequency(1.0)]
...
def test_mode_solver_web_run_batch(mock_remote_api):
    ...
    num_freqs = 1
    num_of_sims = 1
    freqs = np.linspace(td.frequency(wav_min), td.frequency(wav_max), num_freqs)
