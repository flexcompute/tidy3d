...
def test_triangular_dataset_no_vtk(tmp_path, dataset_type_id, log_capture):
    _test_triangular_dataset(log_capture, tmp_path, "test_name", 0, dataset_type_id, no_vtk=True)


@pytest.mark.usefixtures("hide_vtk")
@pytest.mark.parametrize("dataset_type_ind", [0, 1])
def test_tetrahedral_dataset_no_vtk(tmp_path, dataset_type_id, log_capture):
    _test_tetrahedral_dataset(log_capture, tmp_path, "test_name", 0, dataset_type_id, no_vtk=True)
