...
from tidy3d.components.mode import ModeSolver
from tidy3d.components.mode.mode_solver import MODE_MONITOR_NAME
...
def test_mode_solver_straight_vs_angled():
    ...
    ms_angled = ModeSolver.from_simulation(
...
def test_mode_solver_2D():
    ...
    ms = ModeSolver.from_simulation(simulation=simulation, plane=PLANE, mode_spec=mode_spec, freqs=[td.C_0 / 1.0])
    compare_colocation(ms)
    verify_pol_fraction(ms)
    check_ms_reduction(ms)
