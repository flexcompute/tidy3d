...
def make_eme_sim():
    ...
    eme_port_grid = td.EMEUniformGrid(num_cells=1, mode_spec=mode_spec.updated_copy(num_modes=2))
...
def test_eme_grid():
    ...
    # test max num cells
    large_grid = td.EMEUniformGrid(num_cells=1000, mode_spec=td.ModeSpec())
    with pytest.raises(pd.ValidationError):
        _ = large_grid.make_grid(center=sim_geom.center, size=sim_geom.size, axis=axis)
    too_many_modes = td.EMEUniformGrid(num_cells=1, mode_spec=td.ModeSpec(num_modes=1000))
    with pytest.raises(pd.ValidationError):
        _ = too_many_modes.make_grid(center=sim_geom.center, size=sim_geom.size, axis=axis)

    # test unsupported mode specs
    bad_mode_spec = td.EMEUniformGrid(num_cells=1, mode_spec=td.ModeSpec(angle_theta=0.1))
    with pytest.raises(pd.ValidationError):
        _ = bad_mode_spec.make_grid(center=sim_geom.center, size=sim_geom.size, axis=axis)
    bad_mode_spec = td.EMEUniformGrid(num_cells=1, mode_spec=td.ModeSpec(angle_phi=0.1))
    with pytest.raises(pd.ValidationError):
        _ = bad_mode_spec.make_grid(center=sim_geom.center, size=sim_geom.size, axis=axis)
    bad_mode_spec = td.EMEUniformGrid(num_cells=1, mode_spec=td.ModeSpec(precision="double"))
    with pytest.raises(pd.ValidationError):
        _ = bad_mode_spec.make_grid(center=sim_geom.center, size=sim_geom.size, axis=axis)
...
def test_eme_simulation(log_capture):  # noqa: F811
    ...
    with pytest.raises(SetupError):
        _ = sim.updated_copy(monitors=[monitor])
    monitor = sim.monitors[2].updated_copy(num_modes=3)
    with pytest.raises(SetupError):
        _ = sim.updated_copy(monitors=[monitor])
    ...
    with pytest.raises(ValidationError):
        _ = sim.updated_copy(
            size=(10, 10, 10), monitors=[large_monitor], freqs=list(1e14 * np.linspace(1, 2, 50))
        )
...
def _get_eme_scalar_mode_field_data_array():
    ...
    return td.EMEScalarFieldDataArray(
        (1 + 1j) * np.random.random((len(x), len(y), 1, 2, 5, 5)), coords=coords
    )
...
def _get_eme_smatrix_data_array(num_modes_in=2, num_modes_out=3, num_freqs=2):
    mode_index_in = np.arange(num_modes_in)
    mode_index_out = np.arange(num_modes_out)
    f = td.C_0 * np.linspace(1, 2, num_freqs)
    if num_modes_in == 0 and num_modes_out == 0:
        data = (1 + 1j) * np.random.random(len(f))
        coords = dict(f=f)
        return xr.DataArray(data, coords=coords, dims=["f"])
    elif num_modes_in == 0 and num_modes_out != 0:
        data = (1 + 1j) * np.random.random((len(f), len(mode_index_out)))
        coords = dict(f=f, mode_index_out=mode_index_out)
        return xr.DataArray(data, coords=coords, dims=["f", "mode_index_out"])
    elif num_modes_in != 0 and num_modes_out == 0:
        data = (1 + 1j) * np.random.random((len(f), len(mode_index_in)))
        coords = dict(f=f, mode_index_in=mode_index_in)
        return xr.DataArray(data, coords=coords, dims=["f", "mode_index_in"])
    else:
        data = (1 + 1j) * np.random.random((len(f), len(mode_index_out), len(mode_index_in)))
        coords = dict(f=f, mode_index_out=mode_index_out, mode_index_in=mode_index_in)
        return td.EMESMatrixDataArray(data, coords=coords)


def _get_eme_smatrix_dataset(num_modes_1=3, num_modes_2=4):
    S11 = _get_eme_smatrix_data_array(num_modes_in=num_modes_1, num_modes_out=num_modes_1)
    S12 = _get_eme_smatrix_data_array(num_modes_in=num_modes_2, num_modes_out=num_modes_1)
    S21 = _get_eme_smatrix_data_array(num_modes_in=num_modes_1, num_modes_out=num_modes_2)
    S22 = _get_eme_smatrix_data_array(num_modes_in=num_modes_2, num_modes_out=num_modes_2)
...
def _get_eme_mode_solver_data():
    ...
    return td.EMEModeSolverData(monitor=monitor, **kwargs)
...
def _get_mode_solver_data(modes_out=False, num_modes=3):
    ...
    eme_mode_data = _get_eme_mode_solver_dataset()
    kwargs = eme_mode_data.field_components
...
def _get_eme_port_modes():
    ...
    kwargs = mode_data.field_components
    kwargs = {
        key: field.isel(eme_cell_index=[0, len(n_complex.eme_cell_index) - 1])
