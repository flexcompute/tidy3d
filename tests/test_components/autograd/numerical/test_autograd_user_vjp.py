# tests user_vjp autograd hook for run_custom and run_async_custom and compares to numerically computed finite difference gradients
...
LOCAL_GRADIENT = True
VERBOSE = True
NUMERICAL_RESULTS_DATA_DIR = "./numerical_user_vjp_test/"
...
def create_objective_function(geometry, create_sim_base, eval_fn, run_fn, sim_path_dir):
    def objective(sphere_parameters_lists):
        ...
        for idx in range(len(sphere_parameters_lists)):
            objective_vals.append(eval_fn(sim_data[f"numerical_user_vjp_testing_{idx}"]))
...
run_functions = ["run_async_custom"]
