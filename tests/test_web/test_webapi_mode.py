...
from tidy3d import ModeSimulation
...
from ..test_components.test_mode import get_mode_sim
...
def test_download_json(monkeypatch, mock_get_info, tmp_path):
    ...
    assert ModeSimulation.from_file(fname_tmp) == sim


@responses.activate
def test_load_simulation(monkeypatch, mock_get_info, tmp_path):
    def mock_download(*args, **kwargs):
        get_mode_sim().to_file(args[1])
