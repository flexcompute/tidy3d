...
def _test_sphere_plot():
    ...
    s3 = Sphere(radius=0.1, center=(0, 0, 1))
...
def _test_polyslab_plot():
    ...
    p1 = PolySlab(vertices=vertices, slab_bounds=(-0.5, 0.5), axis=2)
...
def _test_sim_plot():
    ...
    sim = Simulation(
        geometry=Box(size=(4.0, 4.0, 4.0), center=(0, 0, 0)),
        mesh=Mesh(
        ...
        structures={
            ...
            "box3": Structure(
                geometry=Sphere(radius=0.5, center=(0, 1.5, 0)),
                medium=Medium(permittivity=2.0),
            ),
            "box4": Structure(
                geometry=Cylinder(radius=0.5, length=1, center=(0, -1.5, 0)),
                medium=Medium(permittivity=2.0),
            ),
        },
        sources={
            "plane_source": Source(
                geometry=Box(size=(4, 4, 0.0), center=(0, 0, -1.5)),
                source_time=Pulse(
                    freq0=1e14,
                    fwidth=1e12,
                ),
