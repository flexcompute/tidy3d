...
.. list-table::
   ...
   * - Save and load any Tidy3d object?
     - If the object ``obj`` is an instance of ``ObjClass``, save and load with ``obj.to_file(path='fname/to/file.json')`` and ``obj = ObjClass.from_file(fname='path/to/file.json')``, respectively.
