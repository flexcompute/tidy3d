...
def generate_material_library_doc():
    ...
    with open(fname, 'w') as f:
        ...
        for abbr, mat in sorted(lib.items(), key=lambda item: item[1].name): # iterate materials
            ...
            for varname, var in sorted(mat.variants.items(), key=lambda item: item[0]): # iterate variants
                ...
                for col in columns:
                    if width[col] < len(row[col]): width[col] = len(row[col])
                
                # Update example code
                code_string += '>>> medium = material_library[\'' + abbr + '\'][\'' + varname + '\']\n\n'
            ...
            f.write(divider)
            f.write(tab + ' '.join(name[col] + ' ' * (width[col] - len(name[col])) for col in columns) + '\n')
            f.write(divider)
            for row in rows:
                f.write(tab + ' '.join(row[col] + ' ' * (width[col] - len(row[col])) for col in columns) + '\n')
            f.write(divider)
            f.write('\n')
